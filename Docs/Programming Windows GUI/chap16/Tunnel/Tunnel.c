/*---------------------------------------
   TUNNEL.C -- Démo d'animation de la palette
...
TCHAR szTitle   [] = TEXT ("Tunnel : Démo d'animation de la palette") ;
...
HPALETTE CreateRoutine (HWND hwnd)
{
     ...
     plp = malloc (sizeof (LOGPALETTE) + 255 * sizeof (PALETTEENTRY)) ;
          ...
          // Initialise les membres de la structure LOGPALETTE
...
void PaintRoutine (HDC hdc, int cxClient, int cyClient)
{
     ...
     for (i = 0 ; i < 127 ; i++)
     {
               // Utilise une structure RECT pour chacun des 128 rectangles
          ...
          hBrush = CreateSolidBrush (PALETTEINDEX (i)) ;
               ...
               // Remplit le rectangle et supprime le pinceau
