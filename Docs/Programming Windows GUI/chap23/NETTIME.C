/*-------------------------------------------------------
   NETTIME.C -- Réglage de l'horloge système à partir 
                    ...
                    PSTR szCmdLine, int iCmdShow)
{
     ...
     hwnd = CreateWindow (szAppName, TEXT ("Régler l'horloge système par Internet"),
          ...
          // Crée la boîte de dialogue en haut de la fenêtre

     hwndModeless = CreateDialog (hInstance, szAppName, hwnd, MainDlg) ;

          // Dimensionne la fenêtre principale à la taille de la boîte de dialogue.  
     ...
     UpdateWindow (hwnd) ;
          ...
          // Boucle de message normale utilisée avec de grandes boîtes de dialogue.
...
BOOL CALLBACK MainDlg (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
     ...
     case WM_COMMAND:
          ...
          case IDOK:
                                                   ...
                                                   FD_CONNECT | FD_READ))
               {
                    ...
                    // Appelle "connect" avec l'adresse IP  et le port utilisé
               ...
               connect(sock, (SOCKADDR *) &sa, sizeof (sa)) ;
                    ...
                    // "connect" va retourner SOCKET_ERROR car même
                    // en cas de succès il va demander le blocage. Ce qui suit
               ...
               EditPrintf (hwndEdit, TEXT ("Connecting to %hs..."), szIPAddr) ;
                    ...
                    // Le résultat de l'appel "connect" est rapporté
                    // dans le message WM_SOCKET_NOTIFY.
                    // Règle l'horloge et change le bouton en "Annuler"
     ...
     case WM_SOCKET_NOTIFY:
          ...
          wError = WSAGETSELECTERROR (lParam) ;   //  HIWORD
               ...
               // Traite les deux événements spécifiés dans WSAAsyncSelect

          switch (wEvent)
          {
               // Cet événement résulte de l'appel "connect"
          ...
          case FD_CONNECT:
               ...
               EditPrintf (hwndEdit, TEXT ("Connected to %hs.\r\n"), szIPAddr) ;
                    ...
                    // Tente de récupérer les données. L'appel va générer une erreur
                    // de WSAEWOULDBLOCK et un événement de FD_READ

               recv (sock, (char *) &ulTime, 4, MSG_PEEK) ;
               EditPrintf (hwndEdit, TEXT ("Waiting to receive...")) ;
               return TRUE ;

                    // Cela se produit également quand l'appel "recv" peut être fait
          ...
          case FD_READ:
               ...
               if (wError)
               {
                    ...
                    // Récupère l'heure et échange les octets
